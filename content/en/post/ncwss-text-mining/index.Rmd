...
- icon: github
  icon_pack: fab
  name: code
  url: https://github.com/maxwelco/maxwelco-blog/tree/master/content/en/post/ncwss-text-mining
summary: 'Text analysis from 20 years of North Central Weed Science Society Meeting Proceedings'
authors: []
lastmod: '2021-02-10T16:51:54-03:00'
featured: yes
image:
  caption: ''
  focal_point: ''
...
```{r name-pdfs}
pdfs <- str_c("docs", "/", list.files("docs", pattern = "*.pdf"), sep = "")
pdfs 
```
...
pdf1 <- pdf %>% 
  ...
  mutate(text = str_to_lower(text), # making all text lower case
         ...
         text = str_replace(text, "marestail", "horseweed")) %>% 
...
Next I will use functions to *count* words over the years, *arrange* it as descending, *group_by* year, rank them and *filter* the top 100 words by each year. 
...
```{r}
pdf5 <- pdf4 %>% 
  mutate(highlight = if_else(word %in% c("amaranth", "palmer", "kochia", "horseweed", "grass", "nightshade", "waterhemp", "velvetleaf", "ragweed", "sunflower",
                                            "foxtail"), TRUE, FALSE),
...
theme_style <- theme(text = element_text(family = font_family),
                  ...
                  plot.title = element_markdown(family = title_family,
                                            face = 'bold', size = 70, colour = text_colour),
                  plot.subtitle = element_markdown(family = font_family, 
                                                   size = 30, colour = text_colour),
...
#Set colour palette
cols <- c("#F2D9F3", "#F2D9F3", "#00E5E5", "#DEB887", "#FAC8C8", "#39393A", "#FA9664", 
          "#FF4040", "#48DE7A", "#942DC7", "#F5F5DC", "#FAFA00")
```
...
figure <- pdf5 %>% 
  ...
  labs(title = "<b style='color:red;'>NCWSS</b> annual meeting 
       ...
       Common weed species names are highlighed to describe 
       their change across 20 years.", 
       y= "Rank",
       caption = "Visualization: @maxwelco adapted from 
       @JaredBraggins | Source: NCWSS") 
...
I good source to learn [Tidy Text](https://juliasilge.com/blog/learn-tidytext-learnr/) by Julia Silge.
